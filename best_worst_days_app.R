...
ui <- fluidPage(
  ...
  titlePanel("SP500 Daily Percent Change Analysis"),
  
  sidebarLayout(
    sidebarPanel(
      sliderInput("count_var", "Outlier Removal Count:", min = 10, max = 200, value = 100, step = 10),
      selectInput("outlier_level", "Outlier Level:", choices = c("weeks", "months"), selected = "months"),
      actionButton("update", "Update Graphs")
    ),
    
    mainPanel(
      plotOutput("percentChangePlot"),
      plotOutput("sp500ValuePlot"),
...
server <- function(input, output) {
  ...
  observeEvent(input$update, {
    ...
    # Plot 2: SP500 Value
    output$sp500ValuePlot <- renderPlot({
      filtered_data %>%
        mutate(tab = 'Filtered', Close = new_values) %>%
        select(-MATCHED) %>%
        rbind(cleaned_df %>%
    ...
    # Plot 3: Log-transformed SP500 Value
    output$logValuePlot <- renderPlot({
      filtered_data %>%
